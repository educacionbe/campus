...
class MoodletxtXMLBuilder {
    ...
    /**
     * Builds an Opt Out status check request request
     * Method to build an XML request to retrieve the opt out status of a set of phone numbers.
     * @param TxttoolsAccount $txttoolsAccount account to authenticate with
     * @param array() $phoneNumbers
     * @return array An array of completed XML packets to be sent by the connector
     * @version 2015062901
     * @since 2015041501
     */
    public function buildBuildOptOutStatusCheckRequest($txttoolsAccount, $phoneNumbers) {

        $this->buildAuthentication($txttoolsAccount);
        $this->buildOptOutCheckMessageBlocks($phoneNumbers);
...
//        error_log(get_string('logxmlblockcreated', 'block_moodletxt') . "\r\n\r\n" . $this->currentRequest);
    ...
    /**
     * Build Opt Out Check message blocks.
     * Method builds the <Message> blocks to be included in the message being built.
     * These blocks are cached into an array, then compiled into <Request> blocks later on.
     * @param array() $phoneNumbers
     * @version 2015040101
     * @since 2015040101
     */
    private function buildOptOutCheckMessageBlocks($phoneNumbers) {

        // Only build message if we have recipients
        if (count($phoneNumbers) != 0) {

            // This function currently only builds an OptOutCheck message with 
            //  returnBlockedOnly="false", which means every recipient listed
            //  will return status, whether black listed or not.
            $messageBlock = ' 
    ' . MoodletxtXMLConstants::$REQUEST_OPT_OUT_CHECK_BLOCK;

            // Loop over recipients and add their phone numbers in phone blocks
            foreach ($phoneNumbers as $number) {
            
                // Code written this way so the output is all nice and formatted
                $messageBlock .= ' 
        ' . MoodletxtXMLConstants::$REQUEST_MESSAGE_PHONE . $number . MoodletxtXMLConstants::$_REQUEST_MESSAGE_PHONE;
            }
            
            $messageBlock .= '
    ' . MoodletxtXMLConstants::$_REQUEST_OPT_OUT_CHECK_BLOCK;
